...
A consequence worth knowing: a README feature is **dormant**. Truncation recovery is fully dormant — its *detect-and-persist* half (`save_*_truncation`) only ever lived in the dead `streaming_openai.py`, so nothing populates the state map. The *inject* half was removed from both routes (it was inert overhead reading a never-written map); `truncation_state.py` / `truncation_recovery.py` remain as tested-but-dormant modules. The reliable modern truncation signal (a tool call whose JSON arguments don't parse) is instead surfaced directly: the Anthropic streaming path forwards the partial `input_json_delta`, and the non-streaming `collect_anthropic_response` raises a visible error. Likewise `ThinkingParser`/`inject_thinking_tags` (fake reasoning *injection*) are never called by the live path — though upstream models that natively emit `reasoning_content` DO surface as thinking blocks via `streaming_anthropic`/`sse_aggregator`. Treat README's feature list as intent, not as current behavior.
