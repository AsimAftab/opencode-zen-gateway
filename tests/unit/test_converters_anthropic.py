...
from opencode_zen.converters_anthropic import convert_anthropic_content_to_text, extract_system_prompt, extract_tool_results_from_anthropic_content, extract_images_from_tool_results, extract_tool_uses_from_anthropic_content, convert_anthropic_messages, convert_anthropic_tools, anthropic_to_opencode, extract_thinking_config_from_anthropic
from opencode_zen.converters_core import UnifiedMessage, UnifiedTool
from opencode_zen.models_anthropic import AnthropicMessagesRequest, AnthropicMessage, AnthropicTool, TextContentBlock, ToolUseContentBlock, ToolResultContentBlock, SystemContentBlock
...
class TestConvertAnthropicContentToText:
    ...
    def test_ignores_non_text_blocks(self):
        ...
        content = [{'type': 'text', 'text': 'Hello'}, {'type': 'tool_use',
            'id': 'call_123', 'name': 'test', 'input': {}}, {'type': 'text',
            'text': ' World'}]
        print('Action: Extracting text...')
        result = convert_anthropic_content_to_text(content)
        print(f"Comparing result: Expected 'Hello World', Got '{result}'")
...
class TestExtractSystemPrompt:
    ...
    def test_extracts_from_list_with_text_blocks(self):
        ...
        system = [{'type': 'text', 'text': 'You are helpful.'}, {'type':
            'text', 'text': 'Be concise.'}]
        print('Action: Extracting system prompt...')
        result = extract_system_prompt(system)
        print(
    ...
    def test_extracts_from_pydantic_system_content_blocks(self):
        ...
        system = [SystemContentBlock(type='text', text='Part 1'),
    ...
    def test_handles_mixed_content_blocks(self):
        ...
        system = [{'type': 'text', 'text': 'Hello'}, {'type': 'image',
            'source': {'type': 'base64', 'data': '...'}}, {'type': 'text',
            'text': 'World'}]
        print('Action: Extracting system prompt...')
        result = extract_system_prompt(system)
...
class TestExtractToolResultsFromAnthropicContent:
    ...
    def test_extracts_tool_result_from_pydantic_model(self):
        ...
        content = [ToolResultContentBlock(type='tool_result', tool_use_id=
            'call_456', content='Pydantic result')]
        print('Action: Extracting tool results...')
        result = extract_tool_results_from_anthropic_content(content)
        print(f'Result: {result}')
        assert len(result) == 1
    ...
    def test_handles_multiple_images_in_tool_result(self):
        ...
        content = [{'type': 'tool_result', 'tool_use_id': 'call_456',
            'content': [{'type': 'image', 'source': {'type': 'base64',
            'media_type': 'image/png', 'data': 'abc123'}}, {'type': 'image',
    ...
    def test_handles_text_and_image_in_tool_result(self):
        ...
        content = [{'type': 'tool_result', 'tool_use_id': 'call_789',
            'content': [{'type': 'text', 'text': 'Screenshot captured'}, {
            'type': 'image', 'source': {'type': 'base64', 'media_type':
            'image/png', 'data': 'xyz789'}}]}]
        print('Action: Extracting tool results...')
        result = extract_tool_results_from_anthropic_content(content)
        print(f'Result: {result}')
        assert len(result) == 1
...
class TestExtractImagesFromToolResults:
    ...
    def test_extracts_multiple_images_from_tool_result(self):
        ...
        content = [{'type': 'tool_result', 'tool_use_id': 'call_456',
            'content': [{'type': 'image', 'source': {'type': 'base64',
            'media_type': 'image/png', 'data': 'png_data_here'}}, {'type':
            'image', 'source': {'type': 'base64', 'media_type':
            'image/jpeg', 'data': 'jpeg_data_here'}}]}]
        print('Action: Extracting images from tool results...')
        result = extract_images_from_tool_results(content)
        print(f'Result: {result}')
        assert len(result) == 2
    ...
    def test_extracts_images_from_multiple_tool_results(self):
        ...
        content = [{'type': 'tool_result', 'tool_use_id': 'call_1',
            'content': [{'type': 'image', 'source': {'type': 'base64',
            'media_type': 'image/png', 'data': 'first_image'}}]}, {'type':
            'tool_result', 'tool_use_id': 'call_2', 'content': [{'type':
            'image', 'source': {'type': 'base64', 'media_type':
            'image/jpeg', 'data': 'second_image'}}]}]
        print('Action: Extracting images from tool results...')
        result = extract_images_from_tool_results(content)
        print(f'Result: {result}')
        assert len(result) == 2
    ...
    def test_extracts_images_mixed_with_text(self):
        ...
        content = [{'type': 'tool_result', 'tool_use_id': 'call_123',
            'content': [{'type': 'text', 'text': 'Screenshot captured'}, {
            'type': 'image', 'source': {'type': 'base64', 'media_type':
            'image/png', 'data': 'screenshot_data'}}]}]
        print('Action: Extracting images from tool results...')
        result = extract_images_from_tool_results(content)
        print(f'Result: {result}')
        assert len(result) == 1
...
class TestExtractToolUsesFromAnthropicContent:
    ...
    def test_extracts_tool_use_from_dict(self):
        ...
        content = [{'type': 'tool_use', 'id': 'call_123', 'name':
            'get_weather', 'input': {'location': 'Moscow'}}]
    ...
    def test_extracts_tool_use_from_pydantic_model(self):
        ...
        content = [ToolUseContentBlock(type='tool_use', id='call_456', name
    ...
    def test_extracts_multiple_tool_uses(self):
        ...
        content = [{'type': 'tool_use', 'id': 'call_1', 'name': 'tool1',
            'input': {}}, {'type': 'text', 'text': 'Some text'}, {'type':
            'tool_use', 'id': 'call_2', 'name': 'tool2', 'input': {}}]
        print('Action: Extracting tool uses...')
        result = extract_tool_uses_from_anthropic_content(content)
        print(f'Result: {result}')
        assert len(result) == 2
...
class TestConvertAnthropicMessages:
    ...
    def test_converts_user_message_with_content_blocks(self):
        ...
        messages = [AnthropicMessage(role='user', content=[{'type': 'text',
            'text': 'Part 1'}, {'type': 'text', 'text': ' Part 2'}])]
        print('Action: Converting messages...')
        result = convert_anthropic_messages(messages)
        print(f'Result: {result}')
        assert len(result) == 1
    ...
    def test_converts_assistant_message_with_tool_use(self):
        ...
        messages = [AnthropicMessage(role='assistant', content=[{'type':
            'text', 'text': "I'll check the weather"}, {'type': 'tool_use',
            'id': 'call_123', 'name': 'get_weather', 'input': {'location':
            'Moscow'}}])]
        print('Action: Converting messages...')
        result = convert_anthropic_messages(messages)
        print(f'Result: {result}')
    ...
    def test_converts_full_conversation(self):
        ...
        messages = [AnthropicMessage(role='user', content='Hello'),
            ...
            'Hi! How can I help?'), AnthropicMessage(role='user', content=
            "What's the weather?")]
        print('Action: Converting messages...')
        result = convert_anthropic_messages(messages)
        print(f'Result: {result}')
    ...
    def test_extracts_images_from_user_message(self):
        ...
        messages = [AnthropicMessage(role='user', content=[{'type': 'text',
            'text': "What's in this image?"}, {'type': 'image', 'source': {
        ...
        assert len(result[0].images
            ) == 1, f'Expected 1 image, got {len(result[0].images)}'
    ...
    def test_images_only_extracted_from_user_role(self):
        ...
        messages = [AnthropicMessage(role='user', content=[{'type': 'text',
            'text': 'Describe this image'}, {'type': 'image', 'source': {
            'type': 'base64', 'media_type': 'image/png', 'data':
            test_image_base64}}]), AnthropicMessage(role='assistant',
    ...
    def test_extracts_multiple_images_from_user_message(self):
        ...
        messages = [AnthropicMessage(role='user', content=[{'type': 'text',
            'text': 'Compare these images'}, {'type': 'image', 'source': {
            'type': 'base64', 'media_type': 'image/jpeg', 'data':
            test_image_base64}}, {'type': 'image', 'source': {'type':
            'base64', 'media_type': 'image/png', 'data': test_image_base64}
            }, {'type': 'image', 'source': {'type': 'base64', 'media_type':
            'image/webp', 'data': test_image_base64}}])]
        print('Action: Converting messages...')
        result = convert_anthropic_messages(messages)
        print(
        ...
        assert result[0].images is not None
        assert len(result[0].images
            ) == 3, f'Expected 3 images, got {len(result[0].images)}'
    ...
    def test_counts_images_in_debug_log(self, caplog):
        ...
        messages = [AnthropicMessage(role='user', content=[{'type': 'text',
            'text': 'Analyze this'}, {'type': 'image', 'source': {'type':
            'base64', 'media_type': 'image/jpeg', 'data': test_image_base64
            }}, {'type': 'image', 'source': {'type': 'base64', 'media_type':
            'image/png', 'data': test_image_base64}}])]
...
class TestConvertAnthropicTools:
    ...
    def test_converts_tool_from_pydantic_model(self):
        ...
        tools = [AnthropicTool(name='get_weather', description=
    ...
    def test_converts_multiple_tools(self):
        ...
        tools = [AnthropicTool(name='tool1', description='Tool 1',
            input_schema={}), AnthropicTool(name='tool2', description=
            'Tool 2', input_schema={})]
        print('Action: Converting tools...')
        result = convert_anthropic_tools(tools)
        print(f'Result: {result}')
...
class TestAnthropicToOpenCode:
    ...
    def test_builds_simple_payload(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4-5',
            messages=[AnthropicMessage(role='user', content='Hello!')],
            max_tokens=1024)
        print('Action: Converting to OpenCode payload...')
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
            return_value='claude-sonnet-4.5'):
            with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED',
                False):
                result = anthropic_to_opencode(request, 'conv-123',
                    'arn:aws:test')
        print(f'Result: {result}')
        ...
        assert 'userInputMessage' in result['conversationState'][
            'currentMessage']
    ...
    def test_includes_tools(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4-5',
            ...
            "What's the weather?")], max_tokens=1024, tools=[AnthropicTool(
        ...
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
            ...
            with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED',
                ...
                result = anthropic_to_opencode(request, 'conv-123',
                    'arn:aws:test')
        print(f'Result: {result}')
        context = result['conversationState']['currentMessage'][
    ...
    def test_builds_history_for_multi_turn(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4-5',
            ...
            AnthropicMessage(role='assistant', content=
            'Hi! How can I help?'), AnthropicMessage(role='user', content=
            "What's the weather?")], max_tokens=1024)
        print('Action: Converting to OpenCode payload...')
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
            return_value='claude-sonnet-4.5'):
            with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED',
    ...
    def test_handles_tool_use_and_result_flow(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4-5',
            messages=[AnthropicMessage(role='user', content=
            "What's the weather in Moscow?"), AnthropicMessage(role=
            'assistant', content=[{'type': 'text', 'text':
            "I'll check the weather"}, {'type': 'tool_use', 'id':
            'call_123', 'name': 'get_weather', 'input': {'location':
            'Moscow'}}]), AnthropicMessage(role='user', content=[{'type':
            'tool_result', 'tool_use_id': 'call_123', 'content':
            'Weather: Sunny, 25°C'}])], max_tokens=1024, tools=[
            AnthropicTool(name='get_weather', description=
            'Get weather for a location', input_schema={'type': 'object',
            'properties': {'location': {'type': 'string'}}})])
        print('Action: Converting to OpenCode payload...')
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
    ...
    def test_raises_for_empty_messages(self):
        ...
        so empty messages are rejected at the Pydantic level, not at anthropic_to_opencode.
        """
        ...
        with pytest.raises(ValidationError):
            AnthropicMessagesRequest(model='claude-sonnet-4-5', messages=[],
                max_tokens=1024)
        print(
            'ValidationError raised as expected - Pydantic rejects empty messages'
            )
    ...
    def test_injects_thinking_tags_when_enabled(self):
        ...
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
            ...
            with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED',
                True):
                with patch(
                    'opencode_zen.converters_core.FAKE_REASONING_MAX_TOKENS',
                    4000):
                    result = anthropic_to_opencode(request, 'conv-123',
                        'arn:aws:test')
        print(f'Result: {result}')
    ...
    def test_injects_thinking_tags_even_when_tool_results_present(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4-5',
            ...
            )], max_tokens=1024, tools=[AnthropicTool(name='test_tool',
            description='A test tool', input_schema={'type': 'object',
            'properties': {}})])
        print('Action: Converting to OpenCode payload...')
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
            return_value='claude-sonnet-4.5'):
        ...
        print('Checking that thinking tags ARE present...')
        assert '<thinking_mode>enabled</thinking_mode>' in current_content, 'thinking tags SHOULD be injected even with tool results'
        print('Checking that <max_thinking_length> tag IS present...')
        assert '<max_thinking_length>4000</max_thinking_length>' in current_content, 'max_thinking_length tag SHOULD be present even with tool results'
...
class TestExtractThinkingConfigFromAnthropic:
    ...
    def test_thinking_enabled_with_budget(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4.5',
            messages=[AnthropicMessage(role='user', content='test')],
            max_tokens=1024, thinking={'type': 'enabled', 'budget_tokens': 
            8000})
        print('Extracting thinking config...')
        config = extract_thinking_config_from_anthropic(request)
        print(
    ...
    def test_thinking_enabled_without_budget(self):
        ...
        """
        print(
            "Creating request with thinking={'type': 'enabled'} (no budget)..."
            )
        request = AnthropicMessagesRequest(model='claude-sonnet-4.5',
            messages=[AnthropicMessage(role='user', content='test')],
    ...
    def test_thinking_invalid_type(self):
        ...
        request = AnthropicMessagesRequest(model='claude-sonnet-4.5',
            messages=[AnthropicMessage(role='user', content='test')],
            max_tokens=1024, thinking={'type': 'unknown'})
        print('Extracting thinking config...')
        config = extract_thinking_config_from_anthropic(request)
        print(
            f'Comparing: enabled={config.enabled}, budget_tokens={config.budget_tokens}'
...
class TestAnthropicToOpenCodeIntegration:
    ...
    def test_extracts_and_passes_thinking_config(self):
        """
        What it does: Verifies anthropic_to_opencode extracts thinking_config and passes to core
        ...
        with patch('opencode_zen.converters_anthropic.get_model_id_for_kiro',
            ...
            with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED',
                True):
                with patch(
                    'opencode_zen.converters_core.FAKE_REASONING_BUDGET_CAP',
                    10000):
                    payload = anthropic_to_opencode(request,
                        'test-conv-123', 'arn:aws:test')
        print('Extracting userInputMessage content...')
        user_input = payload['conversationState']['currentMessage'][
            'userInputMessage']
