...
from opencode_zen.converters_core import extract_text_content, extract_images_from_content, convert_images_to_opencode_zen_format, merge_adjacent_messages, ensure_first_message_is_user, normalize_message_roles, ensure_alternating_roles, ensure_assistant_before_tool_results, strip_all_tool_content, build_opencode_zen_history, build_opencode_payload, process_tools_with_long_descriptions, inject_thinking_tags, extract_tool_results_from_content, extract_tool_uses_from_message, sanitize_json_schema, convert_tools_to_opencode_zen_format, convert_tool_results_to_opencode_zen_format, tool_calls_to_text, tool_results_to_text, UnifiedMessage, UnifiedTool, ThinkingConfig
TEST_IMAGE_BASE64 = (
...
class TestExtractTextContent:
    ...
    def test_extracts_from_mixed_list(self):
        ...
        content = [{'type': 'text', 'text': 'Part1'}, 'Part2', {'text':
            'Part3'}]
        print('Action: Extracting text...')
        result = extract_text_content(content)
    ...
    def test_extracts_from_pydantic_text_content_block(self):
        ...
        """
        from opencode_zen.models_anthropic import TextContentBlock
    ...
    def test_extracts_from_multiple_pydantic_text_blocks(self):
        ...
        content = [TextContentBlock(type='text', text='Part 1'),
            TextContentBlock(type='text', text=' Part 2'), TextContentBlock
            (type='text', text=' Part 3')]
        print('Action: Extracting text...')
        result = extract_text_content(content)
        print(f"Result: '{result}'")
        print(
            f"Comparing result: Expected 'Part 1 Part 2 Part 3', Got '{result}'"
            )
    ...
    def test_extracts_from_mixed_dict_and_pydantic(self):
        ...
        content = [{'type': 'text', 'text': 'Dict text'}, TextContentBlock(
            type='text', text=' Pydantic text'), ' String text']
        print('Action: Extracting text...')
        result = extract_text_content(content)
        print(f"Result: '{result}'")
        print(
    ...
    def test_extracts_text_ignoring_other_pydantic_types(self):
        ...
        """
        from opencode_zen.models_anthropic import TextContentBlock, ToolUseContentBlock
        print('Setup: Mixed Pydantic content with text and tool_use...')
        content = [TextContentBlock(type='text', text='Before tool'),
            ToolUseContentBlock(type='tool_use', id='call_123', name=
            'test_tool', input={}), TextContentBlock(type='text', text=
            'After tool')]
        print('Action: Extracting text...')
        result = extract_text_content(content)
        print(f"Result: '{result}'")
        print(
            f"Comparing result: Expected 'Before toolAfter tool', Got '{result}'"
            )
    ...
    def test_skips_tool_reference_blocks(self):
        ...
        content = [{'type': 'text', 'text': 'Loaded tools:'}, {'type':
            'tool_reference', 'tool_name': 'mcp__slack__read_channel'}, {
            'type': 'tool_reference', 'tool_name': 'Read'}, {'type': 'text',
            'text': ' done'}]
        print('Action: Extracting text...')
        result = extract_text_content(content)
        print(
            f"Comparing result: Expected 'Loaded tools: done', Got '{result}'")
...
class TestExtractImagesFromContent:
    ...
    def test_extracts_from_anthropic_format_base64(self):
        ...
        content = [{'type': 'text', 'text': 'Describe this image'}, {'type':
            'image', 'source': {'type': 'base64', 'media_type': 'image/png',
    ...
    def test_extracts_from_mixed_content(self):
        ...
        content = [{'type': 'text', 'text': 'Compare these images:'}, {
            ...
            'image/jpeg', 'data': 'image1_data'}}, {'type': 'text', 'text':
            'and'}, {'type': 'image', 'source': {'type': 'base64',
    ...
    def test_handles_invalid_data_url(self):
        ...
        content = [{'type': 'image_url', 'image_url': {'url':
            'data:invalid_format_without_comma'}}]
    ...
    def test_extracts_from_pydantic_image_content_block(self):
        ...
        """
        from opencode_zen.models_anthropic import ImageContentBlock, Base64ImageSource
        print('Setup: Pydantic ImageContentBlock...')
        content = [ImageContentBlock(type='image', source=Base64ImageSource
    ...
    def test_extracts_from_pydantic_url_image_source(self):
        ...
        """
        from opencode_zen.models_anthropic import ImageContentBlock, URLImageSource
        print('Setup: Pydantic ImageContentBlock with URL source...')
        content = [ImageContentBlock(type='image', source=URLImageSource(
            type='url', url='https://example.com/image.gif'))]
        print('Action: Extracting images (should skip URL images)...')
        result = extract_images_from_content(content)
        print(f'Comparing result: Expected [], Got {result}')
        assert result == []
    ...
    def test_extracts_multiple_formats_mixed(self):
        ...
        content = [{'type': 'image_url', 'image_url': {'url':
            f'data:image/jpeg;base64,openai_image_data'}}, {'type': 'image',
    ...
    def test_extracts_webp_format(self):
        ...
        content = [{'type': 'image', 'source': {'type': 'base64',
            'media_type': 'image/webp', 'data': 'webp_data'}}]
        print('Action: Extracting images...')
        result = extract_images_from_content(content)
        print(f'Result: {result}')
        assert len(result) == 1
...
class TestConvertImagesToOpenCodeFormat:
    ...
    def test_converts_multiple_images(self):
        ...
        images = [{'media_type': 'image/jpeg', 'data': 'jpeg_data'}, {
            'media_type': 'image/png', 'data': 'png_data'}, {'media_type':
            'image/gif', 'data': 'gif_data'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
    ...
    def test_skips_images_with_empty_data(self):
        ...
        """
        print('Setup: Image with empty data...')
        images = [{'media_type': 'image/jpeg', 'data': ''}, {'media_type':
            'image/png', 'data': 'valid_data'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
        print(f'Comparing count: Expected 1, Got {len(result)}')
    ...
    def test_extracts_format_from_media_type(self):
        ...
        images = [{'media_type': 'image/jpeg', 'data': 'data1'}, {
            'media_type': 'image/png', 'data': 'data2'}, {'media_type':
            'image/gif', 'data': 'data3'}, {'media_type': 'image/webp',
            'data': 'data4'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
    ...
    def test_strips_data_url_prefix_jpeg(self):
        ...
        images = [{'media_type': 'image/jpeg', 'data':
            f'data:image/jpeg;base64,{pure_base64}'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
        print(
    ...
    def test_strips_data_url_prefix_png(self):
        ...
        images = [{'media_type': 'image/png', 'data':
            f'data:image/png;base64,{pure_base64}'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
        print(
    ...
    def test_extracts_media_type_from_data_url(self):
        ...
        images = [{'media_type': 'image/jpeg', 'data':
            f'data:image/gif;base64,{pure_base64}'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
    ...
    def test_handles_malformed_data_url_no_comma(self):
        ...
        print(
            'Action: Converting to OpenCode format (should handle gracefully)...'
            )
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
        assert len(result) == 1
    ...
    def test_strips_data_url_prefix_webp(self):
        ...
        images = [{'media_type': 'image/webp', 'data':
            f'data:image/webp;base64,{pure_base64}'}]
        print('Action: Converting to OpenCode format...')
        result = convert_images_to_opencode_zen_format(images)
        print(f'Result: {result}')
        assert result[0]['source']['bytes'] == pure_base64
...
class TestMergeAdjacentMessages:
    ...
    def test_preserves_alternating_messages(self):
        ...
        messages = [UnifiedMessage(role='user', content='Hello'),
            UnifiedMessage(role='assistant', content='Hi'), UnifiedMessage(
    ...
    def test_merges_multiple_adjacent_groups(self):
        ...
        messages = [UnifiedMessage(role='user', content='A'),
            UnifiedMessage(role='user', content='B'), UnifiedMessage(role=
            'assistant', content='C'), UnifiedMessage(role='assistant',
            content='D'), UnifiedMessage(role='user', content='E')]
        print('Action: Merging messages...')
        result = merge_adjacent_messages(messages)
        print(f'Comparing length: Expected 3, Got {len(result)}')
        assert len(result) == 3
    ...
    def test_merges_list_contents_correctly(self):
        ...
        messages = [UnifiedMessage(role='user', content=[{'type': 'text',
            'text': 'Part 1'}]), UnifiedMessage(role='user', content=[{
    ...
    def test_merges_adjacent_assistant_tool_calls(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'tooluse_first', 'type': 'function', 'function': {
            'name': 'shell', 'arguments': '{"command": ["ls"]}'}}]),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'tooluse_second', 'type': 'function', 'function': {'name':
            'shell', 'arguments': '{"command": ["pwd"]}'}}])]
        print('Action: Merging messages...')
        result = merge_adjacent_messages(messages)
        print(f'Result: {result}')
        print(f'Comparing length: Expected 1, Got {len(result)}')
    ...
    def test_merges_three_adjacent_assistant_tool_calls(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            ...
            'tool1', 'arguments': '{}'}}]), UnifiedMessage(role='assistant',
            content='', tool_calls=[{'id': 'call_2', 'type': 'function',
            'function': {'name': 'tool2', 'arguments': '{}'}}]),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_3', 'type': 'function', 'function': {'name': 'tool3',
            'arguments': '{}'}}])]
        print('Action: Merging messages...')
        result = merge_adjacent_messages(messages)
        print(f'Result: {result}')
    ...
    def test_merges_assistant_with_and_without_tool_calls(self):
        ...
        """
        print(
            'Setup: Assistant without tool_calls + assistant with tool_calls...'
            )
        messages = [UnifiedMessage(role='assistant', content='Thinking...',
            tool_calls=None), UnifiedMessage(role='assistant', content='',
    ...
    def test_merges_user_messages_with_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content='', tool_results=[{
            'type': 'tool_result', 'tool_use_id': 'call_1', 'content':
            'Result 1'}]), UnifiedMessage(role='user', content='',
            tool_results=[{'type': 'tool_result', 'tool_use_id': 'call_2',
            'content': 'Result 2'}])]
        print('Action: Merging messages...')
        result = merge_adjacent_messages(messages)
        print(f'Result: {result}')
...
class TestEnsureFirstMessageIsUser:
    ...
    def test_prepends_synthetic_user_when_first_is_assistant(self):
        ...
        messages = [UnifiedMessage(role='assistant', content=
            "Hello! I'm here to help."), UnifiedMessage(role='user',
    ...
    def test_handles_single_assistant_message(self):
        ...
        result = ensure_first_message_is_user(messages)
        print(
            f'Comparing length: Expected 2 (synthetic + original), Got {len(result)}'
            )
        assert len(result) == 2
        assert result[0].role == 'user'
        assert result[0].content == '(empty placeholder)'
    ...
    def test_handles_assistant_user_assistant_sequence(self):
        ...
        messages = [UnifiedMessage(role='assistant', content=
            'First response'), UnifiedMessage(role='user', content=
            'Question'), UnifiedMessage(role='assistant', content=
            'Second response')]
        print('Action: Ensuring first message is user...')
        result = ensure_first_message_is_user(messages)
        print(
    ...
    def test_preserves_tool_calls_in_assistant_message(self):
        ...
        messages = [UnifiedMessage(role='assistant', content=
            'Let me check that for you.', tool_calls=[{'id': 'call_123',
    ...
    def test_preserves_images_in_messages(self):
        ...
        messages = [UnifiedMessage(role='assistant', content=
            "What's in this image?"), UnifiedMessage(role='user', content=
            'Here it is', images=[{'media_type': 'image/jpeg', 'data':
            'base64data'}])]
        print('Action: Ensuring first message is user...')
        result = ensure_first_message_is_user(messages)
    ...
    def test_uses_minimal_content_for_synthetic_message(self):
        ...
        assert result[0].content == '(empty placeholder)'
        print(
            '✓ Synthetic message uses minimal content (matches LiteLLM behavior)'
            )
...
class TestNormalizeMessageRoles:
    ...
    def test_converts_developer_role_to_user(self):
        ...
        messages = [UnifiedMessage(role='developer', content=
            '<permissions>sandbox enabled</permissions>'), UnifiedMessage(
        ...
        assert result[0].role == 'user'
        assert result[0
            ].content == '<permissions>sandbox enabled</permissions>'
    ...
    def test_converts_multiple_unknown_roles_to_user(self):
        ...
        messages = [UnifiedMessage(role='developer', content='Dev context'),
    ...
    def test_preserves_tool_calls_when_normalizing(self):
        ...
        messages = [UnifiedMessage(role='developer', content='Context',
    ...
    def test_preserves_tool_results_when_normalizing(self):
        ...
        messages = [UnifiedMessage(role='developer', content='Result',
    ...
    def test_preserves_images_when_normalizing(self):
        ...
        messages = [UnifiedMessage(role='developer', content='Screenshot',
...
class TestEnsureAlternatingRoles:
    ...
    def test_inserts_synthetic_assistant_between_two_consecutive_users(self):
        ...
        print(
            f'Comparing length: Expected 3 (2 user + 1 synthetic), Got {len(result)}'
            )
        assert len(result) == 3
    ...
    def test_inserts_multiple_synthetic_assistants_for_four_consecutive_users(
        self):
        """
        ...
        messages = [UnifiedMessage(role='user', content='First'),
            UnifiedMessage(role='user', content='Second'), UnifiedMessage(
            role='user', content='Third'), UnifiedMessage(role='user',
            content='Fourth')]
        print('Action: Ensuring alternating roles...')
        result = ensure_alternating_roles(messages)
        print(
            f'Comparing length: Expected 7 (4 user + 3 synthetic), Got {len(result)}'
            )
        assert len(result) == 7
        print('Checking alternation pattern...')
        assert result[0].role == 'user' and result[0].content == 'First'
        assert result[1].role == 'assistant' and result[1
            ].content == '(empty placeholder)'
        assert result[2].role == 'user' and result[2].content == 'Second'
        assert result[3].role == 'assistant' and result[3
            ].content == '(empty placeholder)'
        assert result[4].role == 'user' and result[4].content == 'Third'
        assert result[5].role == 'assistant' and result[5
            ].content == '(empty placeholder)'
    ...
    def test_preserves_already_alternating_messages(self):
        ...
        messages = [UnifiedMessage(role='user', content='Hello'),
            UnifiedMessage(role='assistant', content='Hi'), UnifiedMessage(
            role='user', content='How are you?'), UnifiedMessage(role=
            'assistant', content='Fine')]
        print('Action: Ensuring alternating roles...')
        result = ensure_alternating_roles(messages)
    ...
    def test_handles_multiple_groups_of_consecutive_users(self):
        ...
        messages = [UnifiedMessage(role='user', content='A'),
            UnifiedMessage(role='user', content='B'), UnifiedMessage(role=
            'assistant', content='C'), UnifiedMessage(role='user', content=
            'D'), UnifiedMessage(role='user', content='E'), UnifiedMessage(
            role='user', content='F')]
        print('Action: Ensuring alternating roles...')
        result = ensure_alternating_roles(messages)
        print(
        ...
        assert result[7].role == 'assistant' and result[7
            ].content == '(empty placeholder)'
    ...
    def test_preserves_tool_results_in_original_messages(self):
        ...
        messages = [UnifiedMessage(role='user', content='First',
            tool_results=[{'type': 'tool_result', 'tool_use_id': 'call_1',
            'content': 'Result 1'}]), UnifiedMessage(role='user', content=
            'Second', tool_results=[{'type': 'tool_result', 'tool_use_id':
            'call_2', 'content': 'Result 2'}])]
        print('Action: Ensuring alternating roles...')
        result = ensure_alternating_roles(messages)
        print(f'Comparing length: Expected 3, Got {len(result)}')
        assert len(result) == 3
    ...
    def test_preserves_images_in_original_messages(self):
        ...
        messages = [UnifiedMessage(role='user', content='First', images=[{
            'media_type': 'image/png', 'data': 'data1'}]), UnifiedMessage(
            role='user', content='Second', images=[{'media_type':
            'image/jpeg', 'data': 'data2'}])]
        print('Action: Ensuring alternating roles...')
        result = ensure_alternating_roles(messages)
        print(f'Comparing length: Expected 3, Got {len(result)}')
        assert len(result) == 3
...
class TestNormalizeAndAlternatingIntegration:
    ...
    def test_developer_messages_are_normalized_and_alternated(self):
        ...
        messages = [UnifiedMessage(role='developer', content='Context 1'),
    ...
    def test_mixed_roles_are_normalized_and_alternated(self):
        ...
        messages = [UnifiedMessage(role='system', content='System'),
            UnifiedMessage(role='developer', content='Dev'), UnifiedMessage
            (role='user', content='User1'), UnifiedMessage(role='assistant',
            content='Assistant1'), UnifiedMessage(role='developer', content
            ='Dev2'), UnifiedMessage(role='user', content='User2')]
        print('Action: Normalizing roles...')
        normalized = normalize_message_roles(messages)
        print(f'After normalization: {[msg.role for msg in normalized]}')
        print('Action: Ensuring alternating roles...')
...
class TestEnsureAssistantBeforeToolResults:
    ...
    def test_preserves_tool_results_with_preceding_assistant(self):
        ...
        messages = [UnifiedMessage(role='user', content='Call a tool'),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_123', 'type': 'function', 'function': {'name':
            'get_weather', 'arguments': '{"location": "Moscow"}'}}]),
            UnifiedMessage(role='user', content='', tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_123', 'content':
            'Weather is sunny'}])]
        print('Action: Processing messages...')
        result, stripped = ensure_assistant_before_tool_results(messages)
        print(f'Result: {result}')
    ...
    def test_strips_orphaned_tool_results_at_start(self):
        ...
        messages = [UnifiedMessage(role='user', content='', tool_results=[{
            ...
            'Orphaned result'}]), UnifiedMessage(role='user', content=
    ...
    def test_converts_tool_results_after_assistant_without_tool_calls(self):
        ...
        messages = [UnifiedMessage(role='user', content='Hello'),
            UnifiedMessage(role='assistant', content='Let me think...',
            tool_calls=None), UnifiedMessage(role='user', content='',
            tool_results=[{'type': 'tool_result', 'tool_use_id': 'call_123',
            'content': 'Result'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
    ...
    def test_converts_tool_results_after_user_message(self):
        ...
        messages = [UnifiedMessage(role='user', content='First message'),
            UnifiedMessage(role='user', content='', tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_123', 'content': 'Result'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
        print(f'Result: {result}')
        print('Checking that tool_results are converted to text...')
    ...
    def test_preserves_content_when_converting_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Here is some context', tool_results=[{'type': 'tool_result',
        ...
        print(
            'Checking that tool_results are converted to text and appended...')
    ...
    def test_preserves_tool_calls_when_converting_tool_results(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'call_new', 'type': 'function', 'function': {'name':
            'new_tool', 'arguments': '{}'}}], tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_old', 'content':
            'Old result'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
        print(f'Result: {result}')
    ...
    def test_handles_multiple_orphaned_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content='', tool_results=[{
            'type': 'tool_result', 'tool_use_id': 'call_1', 'content':
            'Result 1'}, {'type': 'tool_result', 'tool_use_id': 'call_2',
            'content': 'Result 2'}, {'type': 'tool_result', 'tool_use_id':
            'call_3', 'content': 'Result 3'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
        print(f'Result: {result}')
        print(f"Content after conversion: '{result[0].content}'")
    ...
    def test_conversion_preserves_images(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            "Here's an image and tool result", images=[{'media_type':
            'image/jpeg', 'data': 'image_data'}], tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_123', 'content':
            'Tool output'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
    ...
    def test_conversion_appends_to_existing_content(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Original content here', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_abc', 'content': 'Tool data'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
    ...
    def test_conversion_returns_correct_flag(self):
        ...
        """
        print(
            'Setup: Two scenarios - with and without orphaned tool_results...')
        messages_with_orphaned = [UnifiedMessage(role='user', content=
            'Test', tool_results=[{'type': 'tool_result', 'tool_use_id':
            'call_1', 'content': 'Result'}])]
        ...
        messages_without_orphaned = [UnifiedMessage(role='user', content=
            'Hello'), UnifiedMessage(role='assistant', content='',
            tool_calls=[{'id': 'call_1', 'type': 'function', 'function': {
            'name': 'tool', 'arguments': '{}'}}]), UnifiedMessage(role=
            'user', content='', tool_results=[{'type': 'tool_result',
    ...
    def test_normal_tool_results_unchanged(self):
        ...
        messages = [UnifiedMessage(role='user', content='Call a tool'),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_valid', 'type': 'function', 'function': {'name':
            'test_tool', 'arguments': '{}'}}]), UnifiedMessage(role='user',
            content='', tool_results=[{'type': 'tool_result', 'tool_use_id':
            'call_valid', 'content': 'Tool executed successfully'}])]
        print('Action: Processing messages...')
        result, converted = ensure_assistant_before_tool_results(messages)
        print(f'Result: {result}')
        ...
        assert result[2].tool_results[0]['content'
            ] == 'Tool executed successfully'
        print('Checking that content is NOT modified...')
        assert result[2].content == ''
        assert '[Tool Result' not in result[2].content
    ...
    def test_mixed_valid_and_orphaned_tool_results(self):
        ...
        """
        print(
            'Setup: Mixed conversation with valid and orphaned tool_results...'
            )
        messages = [UnifiedMessage(role='user', content='', tool_results=[{
            'type': 'tool_result', 'tool_use_id': 'call_orphan', 'content':
            'Orphaned'}]), UnifiedMessage(role='assistant', content='',
            tool_calls=[{'id': 'call_valid', 'type': 'function', 'function':
            {'name': 'valid_tool', 'arguments': '{}'}}]), UnifiedMessage(
            role='user', content='', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_valid', 'content': 'Valid result'}])]
        print('Action: Processing messages...')
        result, stripped = ensure_assistant_before_tool_results(messages)
        print(f'Result: {result}')
...
class TestSanitizeJsonSchema:
    ...
    def test_preserves_non_empty_required_array(self):
        ...
        schema = {'type': 'object', 'properties': {'location': {'type':
    ...
    def test_recursively_sanitizes_nested_properties(self):
        ...
        schema = {'type': 'object', 'properties': {'nested': {'type':
    ...
    def test_sanitizes_items_in_lists(self):
        ...
        schema = {'anyOf': [{'type': 'string', 'additionalProperties': 
            False}, {'type': 'number', 'required': []}]}
        print('Action: Sanitizing schema...')
        result = sanitize_json_schema(schema)
        print(f'Result: {result}')
    ...
    def test_complex_real_world_schema(self):
        ...
        schema = {'type': 'object', 'properties': {'question': {'type':
            'string', 'description': 'The question to ask'}, 'options': {
            'type': 'string', 'description': 'Array of options'}},
        ...
        assert 'additionalProperties' not in result
        assert result['required'] == ['question', 'options']
...
class TestExtractToolResults:
    ...
    def test_extracts_multiple_tool_results(self):
        ...
        content = [{'type': 'tool_result', 'tool_use_id': 'call_1',
            'content': 'Result 1'}, {'type': 'text', 'text': 'Some text'},
...
class TestConvertToolResultsToOpenCodeFormat:
    ...
    def test_converts_multiple_tool_results(self):
        ...
        tool_results = [{'type': 'tool_result', 'tool_use_id': 'call_1',
            'content': 'Result 1'}, {'type': 'tool_result', 'tool_use_id':
            'call_2', 'content': 'Result 2'}, {'type': 'tool_result',
            'tool_use_id': 'call_3', 'content': 'Result 3'}]
        print('Action: Converting to OpenCode format...')
        result = convert_tool_results_to_opencode_zen_format(tool_results)
        print(f'Result: {result}')
        print(f'Comparing count: Expected 3, Got {len(result)}')
    ...
    def test_extracts_text_from_list_content(self):
        ...
        tool_results = [{'type': 'tool_result', 'tool_use_id': 'call_123',
            'content': [{'type': 'text', 'text': 'Part 1'}, {'type': 'text',
            'text': ' Part 2'}]}]
        print('Action: Converting to OpenCode format...')
        result = convert_tool_results_to_opencode_zen_format(tool_results)
        print(f'Result: {result}')
...
class TestExtractToolUses:
    ...
    def test_extracts_from_tool_calls_field(self):
        ...
        result = extract_tool_uses_from_message(content='', tool_calls=
            tool_calls)
        print(f'Result: {result}')
        assert len(result) == 1
    ...
    def test_extracts_from_content_list(self):
        ...
        content = [{'type': 'tool_use', 'id': 'call_456', 'name': 'search',
            'input': {'query': 'test'}}]
        print('Action: Extracting tool uses...')
        result = extract_tool_uses_from_message(content=content, tool_calls
            =None)
        print(f'Result: {result}')
        assert len(result) == 1
    ...
    def test_returns_empty_for_no_tool_uses(self):
        ...
        print('Action: Extracting tool uses...')
        result = extract_tool_uses_from_message(content='Hello', tool_calls
            =None)
        print(f'Comparing result: Expected [], Got {result}')
        assert result == []
    ...
    def test_extracts_from_both_sources(self):
        ...
        tool_calls = [{'id': 'call_1', 'function': {'name': 'tool1',
            'arguments': '{}'}}]
        content = [{'type': 'tool_use', 'id': 'call_2', 'name': 'tool2',
            'input': {}}]
        print('Action: Extracting tool uses...')
        result = extract_tool_uses_from_message(content=content, tool_calls
            =tool_calls)
...
class TestProcessToolsWithLongDescriptions:
    ...
    def test_returns_none_and_empty_string_for_none_tools(self):
        ...
        print(
            f"Comparing result: Expected (None, ''), Got ({processed}, '{doc}')"
            )
    ...
    def test_short_description_unchanged(self):
        ...
        tools = [UnifiedTool(name='get_weather', description=
            'Get weather for a location', input_schema={'type': 'object',
            'properties': {}})]
        print('Action: Processing tools...')
    ...
    def test_long_description_moved_to_system_prompt(self):
        ...
        tools = [UnifiedTool(name='bash', description=long_description,
        ...
        assert len(processed) == 1
        assert "[Full documentation in system prompt under '## Tool: bash']" in processed[
            0].description
    ...
    def test_mixed_short_and_long_descriptions(self):
        ...
        long_desc = 'B' * 15000
        tools = [UnifiedTool(name='short_tool', description=short_desc,
            input_schema={}), UnifiedTool(name='long_tool', description=
            long_desc, input_schema={})]
        print('Action: Processing tools...')
        with patch('opencode_zen.converters_core.TOOL_DESCRIPTION_MAX_LENGTH',
            10000):
            processed, doc = process_tools_with_long_descriptions(tools)
        ...
        assert '[Full documentation in system prompt' in processed[1
            ].description
    ...
    def test_disabled_when_limit_is_zero(self):
        ...
        long_desc = 'D' * 15000
        tools = [UnifiedTool(name='test_tool', description=long_desc,
            input_schema={})]
    ...
    def test_multiple_long_descriptions_all_moved(self):
        ...
        tools = [UnifiedTool(name='tool1', description='F' * 15000,
            input_schema={}), UnifiedTool(name='tool2', description='G' * 
            15000, input_schema={}), UnifiedTool(name='tool3', description=
            'H' * 15000, input_schema={})]
        print('Action: Processing tools...')
        with patch('opencode_zen.converters_core.TOOL_DESCRIPTION_MAX_LENGTH',
            10000):
            processed, doc = process_tools_with_long_descriptions(tools)
    ...
    def test_empty_description_unchanged(self):
        ...
        tools = [UnifiedTool(name='empty_desc_tool', description='',
            input_schema={})]
        print('Action: Processing tools...')
        with patch('opencode_zen.converters_core.TOOL_DESCRIPTION_MAX_LENGTH',
            10000):
    ...
    def test_none_description_unchanged(self):
        ...
        tools = [UnifiedTool(name='none_desc_tool', description=None,
            input_schema={})]
        print('Action: Processing tools...')
        with patch('opencode_zen.converters_core.TOOL_DESCRIPTION_MAX_LENGTH',
            10000):
        ...
        assert processed[0].description is None or processed[0
            ].description == ''
        assert doc == ''
    ...
    def test_preserves_tool_input_schema(self):
        ...
        input_schema = {'type': 'object', 'properties': {'location': {
            'type': 'string', 'description': 'City name'}, 'units': {'type':
            'string', 'enum': ['celsius', 'fahrenheit']}}, 'required': [
            'location']}
        tools = [UnifiedTool(name='weather', description='C' * 15000,
...
class TestConvertToolsToOpenCodeFormat:
    ...
    def test_replaces_empty_description_with_placeholder(self):
        ...
        """
        print('Setup: Tool with empty description...')
        tools = [UnifiedTool(name='focus_chain', description='',
            input_schema={})]
        print('Action: Converting tools...')
        result = convert_tools_to_opencode_zen_format(tools)
        print(f'Result: {result}')
    ...
    def test_replaces_none_description_with_placeholder(self):
        ...
        """
        print('Setup: Tool with None description...')
        tools = [UnifiedTool(name='test_tool', description=None,
            input_schema={})]
        print('Action: Converting tools...')
        result = convert_tools_to_opencode_zen_format(tools)
        print(f'Result: {result}')
    ...
    def test_sanitizes_input_schema(self):
        ...
        tools = [UnifiedTool(name='test_tool', description='Test',
...
class TestInjectThinkingTags:
    ...
    def test_returns_original_content_when_disabled(self):
        ...
        content = 'Hello, world!'
        print(
            'Action: Inject thinking tags with FAKE_REASONING_ENABLED=False...'
            )
    ...
    def test_injects_tags_when_enabled(self):
        ...
        content = 'What is 2+2?'
        print(
            'Action: Inject thinking tags with FAKE_REASONING_ENABLED=True...')
    ...
    def test_uses_configured_max_tokens(self):
        ...
        content = 'Test'
        print(
            'Action: Inject thinking tags with FAKE_REASONING_MAX_TOKENS=16000...'
            )
        with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED', True
            ):
            with patch('opencode_zen.converters_core.FAKE_REASONING_MAX_TOKENS'
                , 16000):
                with patch(
                    'opencode_zen.converters_core.FAKE_REASONING_BUDGET_CAP', 0
                    ):
                    result = inject_thinking_tags(content, ThinkingConfig())
...
class TestBuildOpenCodeHistory:
    ...
    def test_expects_normalized_roles_only(self):
        ...
        messages = [UnifiedMessage(role='user', content='Normalized user'),
    ...
    def test_builds_user_message_with_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Here are the results', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_123', 'content': 'Result text'}])]
        print('Action: Building history...')
        result = build_opencode_zen_history(messages, 'claude-sonnet-4')
        print(f'Result: {result}')
        assert len(result) == 1
    ...
    def test_builds_assistant_message_with_tool_calls(self):
        ...
        print('Setup: Assistant message with tool_calls...')
        messages = [UnifiedMessage(role='assistant', content=
            "I'll call a tool", tool_calls=[{'id': 'call_123', 'function':
    ...
    def test_mixed_empty_and_non_empty_content_in_history(self):
        ...
        messages = [UnifiedMessage(role='user', content='Start'),
            UnifiedMessage(role='assistant', content=''), UnifiedMessage(
            role='user', content=''), UnifiedMessage(role='assistant',
            content='Response')]
        print('Action: Building history...')
        result = build_opencode_zen_history(messages, 'claude-sonnet-4')
        print(f'Result: {result}')
        ...
        print(
            f"Message 0 content: '{result[0]['userInputMessage']['content']}'")
        ...
        print(
            f"Message 2 content: '{result[2]['userInputMessage']['content']}'")
    ...
    def test_builds_user_message_with_images(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            "What's in this image?", images=[{'media_type': 'image/jpeg',
            'data': TEST_IMAGE_BASE64}])]
        print('Action: Building history...')
        result = build_opencode_zen_history(messages, 'claude-sonnet-4')
        print(f'Result: {result}')
    ...
    def test_builds_user_message_with_multiple_images(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Compare these images', images=[{'media_type': 'image/jpeg',
            'data': 'image1_data'}, {'media_type': 'image/png', 'data':
            'image2_data'}])]
        print('Action: Building history...')
        result = build_opencode_zen_history(messages, 'claude-sonnet-4')
        print(f'Result: {result}')
    ...
    def test_builds_user_message_with_images_and_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            "Here's the image and tool result", images=[{'media_type':
            'image/png', 'data': 'image_data'}], tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_123', 'content':
            'Tool output'}])]
        print('Action: Building history...')
        result = build_opencode_zen_history(messages, 'claude-sonnet-4')
    ...
    def test_builds_user_message_with_webp_image(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Analyze this WebP image', images=[{'media_type': 'image/webp',
    ...
    def test_builds_user_message_with_gif_image(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            "What's happening in this GIF?", images=[{'media_type':
            'image/gif', 'data': 'gif_image_data'}])]
        print('Action: Building history...')
        result = build_opencode_zen_history(messages, 'claude-sonnet-4')
        print(f'Result: {result}')
        user_msg = result[0]['userInputMessage']
...
class TestStripAllToolContent:
    ...
    def test_strips_tool_results_from_user(self):
        ...
        print(f'Result: {result}')
        print(
            'Checking that tool_results are stripped and converted to text...')
        assert len(result) == 1
        assert result[0].tool_results is None
    ...
    def test_strips_both_tool_calls_and_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content='Call a tool'),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_123', 'type': 'function', 'function': {'name':
            'get_weather', 'arguments': '{}'}}]), UnifiedMessage(role=
            'user', content='', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_123', 'content': 'Result'}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
    ...
    def test_strips_multiple_tool_calls(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'call_1', 'type': 'function', 'function': {'name':
            'tool1', 'arguments': '{}'}}, {'id': 'call_2', 'type':
            'function', 'function': {'name': 'tool2', 'arguments': '{}'}},
    ...
    def test_preserves_message_content_when_stripping(self):
        ...
        messages = [UnifiedMessage(role='assistant', content=
            'Let me help you with that', tool_calls=[{'id': 'call_123',
            'type': 'function', 'function': {'name': 'helper', 'arguments':
            '{}'}}]), UnifiedMessage(role='user', content=
            'Thanks for the result', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_123', 'content': 'Done'}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
        print(f'Result: {result}')
        print(
    ...
    def test_preserves_message_role_when_stripping(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'call_1', 'type': 'function', 'function': {'name':
            'tool', 'arguments': '{}'}}]), UnifiedMessage(role='user',
            content='', tool_results=[{'type': 'tool_result', 'tool_use_id':
            'call_1', 'content': 'Result'}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
    ...
    def test_mixed_messages_with_and_without_tool_content(self):
        ...
        messages = [UnifiedMessage(role='user', content='Hello'),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_1', 'type': 'function', 'function': {'name': 'tool',
            'arguments': '{}'}}]), UnifiedMessage(role='user', content=
            '(empty placeholder)')]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
        print(f'Result: {result}')
    ...
    def test_adds_tool_text_for_empty_content_with_tool_calls(self):
        ...
        """
        print(
            'Setup: Assistant message with only tool_calls (empty content)...')
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'call_123', 'type': 'function', 'function': {'name':
            'read_file', 'arguments': '{"path": "test.py"}'}}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
        print(f'Result: {result}')
        print(f"Content after stripping: '{result[0].content}'")
        print(
            'Checking that tool_calls are converted to text representation...')
    ...
    def test_adds_tool_text_for_empty_content_with_tool_results(self):
        ...
        print(f"Content after stripping: '{result[0].content}'")
        print(
            'Checking that tool_results are converted to text representation...'
            )
        assert '[Tool Result' in result[0].content
        assert 'call_123' in result[0].content
    ...
    def test_preserves_existing_content_when_stripping_tool_calls(self):
        ...
        messages = [UnifiedMessage(role='assistant', content=
            "I'll read the file for you", tool_calls=[{'id': 'call_123',
    ...
    def test_preserves_existing_content_when_stripping_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Here are the results you requested', tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_123', 'content':
            'Result data'}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
    ...
    def test_both_tool_calls_and_results_converted_to_text(self):
        ...
        """
        print(
            'Setup: Message with both tool_calls and tool_results (edge case)...'
            )
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'call_1', 'type': 'function', 'function': {'name':
            'my_tool', 'arguments': '{"x": 1}'}}], tool_results=[{'type':
            'tool_result', 'tool_use_id': 'call_0', 'content':
            'Previous result'}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
        print(f'Result: {result}')
        print(f"Content after stripping: '{result[0].content}'")
        print(
            'Checking that both tool_calls and tool_results are converted to text...'
            )
    ...
    def test_multiple_messages_with_empty_content_get_text_representation(self
        ...
        messages = [UnifiedMessage(role='user', content='Read these files'),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_1', 'type': 'function', 'function': {'name': 'read_file',
            'arguments': '{"path": "a.txt"}'}}]), UnifiedMessage(role=
            'user', content='', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_1', 'content': 'File content ABC'}]),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_2', 'type': 'function', 'function': {'name': 'write_file',
            'arguments': '{"path": "b.txt"}'}}]), UnifiedMessage(role=
            'user', content='', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_2', 'content': 'Write completed'}])]
        print('Action: Stripping tool content...')
        result, had_content = strip_all_tool_content(messages)
        print(f'Result: {result}')
        print('Checking text representation for each message...')
        print(f"Message 0 content: '{result[0].content}'")
        assert result[0].content == 'Read these files'
        print(f"Message 1 content: '{result[1].content}'")
        assert '[Tool: read_file' in result[1].content
        ...
        assert '[Tool: write_file' in result[3].content
        assert 'call_2' in result[3].content
        print(f"Message 4 content: '{result[4].content}'")
        assert '[Tool Result' in result[4].content
    ...
    def test_converts_tool_results_to_text_representation(self):
        ...
        print('Setup: User message with tool_results...')
        messages = [UnifiedMessage(role='user', content='', tool_results=[{
            'type': 'tool_result', 'tool_use_id': 'call_xyz789', 'content':
            """File contents:
def hello():
    print('world')"""}])]
        print('Action: Stripping tool content...')
...
class TestStripAllToolContentPreservesImages:
    ...
    def test_preserves_images_when_stripping_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content='Screenshot result',
            tool_results=[{'type': 'tool_result', 'tool_use_id': 'call_123',
            'content': 'Done'}], images=[{'media_type': 'image/png', 'data':
            'screenshot_data'}])]
        print('Action: Stripping tool content...')
    ...
    def test_preserves_images_when_stripping_tool_calls(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='Using tool',
            tool_calls=[{'id': 'call_456', 'type': 'function', 'function':
            {'name': 'test', 'arguments': '{}'}}], images=[{'media_type':
            'image/jpeg', 'data': 'image_data'}])]
        print('Action: Stripping tool content...')
        result, had_tools = strip_all_tool_content(messages)
        print(f'Result: {result}')
        print(f'Images preserved: {result[0].images}')
    ...
    def test_preserves_images_when_stripping_both_tool_calls_and_results(self):
        ...
        messages = [UnifiedMessage(role='user', content='Complex message',
            tool_calls=[{'id': 'call_1', 'type': 'function', 'function': {
            'name': 'tool1', 'arguments': '{}'}}], tool_results=[{'type':
    ...
    def test_preserves_none_images_when_stripping(self):
        ...
        messages = [UnifiedMessage(role='user', content='Text only',
            tool_results=[{'type': 'tool_result', 'tool_use_id': 'call_789',
            'content': 'Done'}], images=None)]
        print('Action: Stripping tool content...')
        result, had_tools = strip_all_tool_content(messages)
        print(f'Result: {result}')
    ...
    def test_preserves_multiple_images_when_stripping(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Multiple screenshots', tool_results=[{'type': 'tool_result',
            'tool_use_id': 'call_multi', 'content': 'Done'}], images=[{
            'media_type': 'image/png', 'data': 'image1'}, {'media_type':
            'image/jpeg', 'data': 'image2'}, {'media_type': 'image/webp',
            'data': 'image3'}])]
        print('Action: Stripping tool content...')
        result, had_tools = strip_all_tool_content(messages)
        print(f'Result: {result}')
...
class TestToolCallsToText:
    ...
    def test_converts_multiple_tool_calls_to_text(self):
        ...
        tool_calls = [{'id': 'call_1', 'type': 'function', 'function': {
            'name': 'read_file', 'arguments': '{"path": "a.txt"}'}}, {'id':
            'call_2', 'type': 'function', 'function': {'name': 'write_file',
    ...
    def test_includes_tool_id_in_output(self):
        ...
        tool_calls = [{'id': 'tooluse_abc123xyz', 'type': 'function',
    ...
    def test_handles_complex_json_arguments(self):
        ...
        complex_args = (
            '{"files": ["a.py", "b.py"], "options": {"recursive": true}}')
        tool_calls = [{'id': 'call_123', 'type': 'function', 'function': {
            'name': 'process', 'arguments': complex_args}}]
        print('Action: Converting to text...')
        result = tool_calls_to_text(tool_calls)
        print(f"Result: '{result}'")
...
class TestToolResultsToText:
    ...
    def test_includes_tool_use_id_in_output(self):
        ...
        tool_results = [{'type': 'tool_result', 'tool_use_id':
            'tooluse_xyz789abc', 'content': 'Done'}]
        print('Action: Converting to text...')
        result = tool_results_to_text(tool_results)
        print(f"Result: '{result}'")
...
class TestBuildOpenCodePayloadIssue20:
    ...
    def test_compaction_without_tools_converts_tool_content_to_text(self):
        ...
        Purpose: Ensure build_opencode_payload doesn't crash and converts tool content to text.
        ...
        messages = [UnifiedMessage(role='user', content=
            'Read the file test.py'), UnifiedMessage(role='assistant',
            content='', tool_calls=[{'id': 'tooluse_abc123', 'type':
            'function', 'function': {'name': 'read_file', 'arguments':
            '{"path": "test.py"}'}}]), UnifiedMessage(role='user', content=
            '', tool_results=[{'type': 'tool_result', 'tool_use_id':
            'tooluse_abc123', 'content':
            """def hello():
    print('world')"""}]), UnifiedMessage(role=
            'assistant', content=
            'I see the file contains a hello function.'), UnifiedMessage(
            role='user', content='Summarize what we did')]
        print(
            'Action: Building OpenCode payload WITHOUT tools (compaction scenario)...'
            )
        result = build_opencode_payload(messages=messages, system_prompt=
            'You are a helpful assistant.', model_id='claude-sonnet-4',
            tools=None, conversation_id='test-conv-123', profile_arn=
        ...
        print(
            'Checking that tool content was converted to text (preserved context)...'
            )
        ...
        for msg in history:
            if 'assistantResponseMessage' in msg:
                ...
                if '[Tool: read_file' in content:
                    ...
                    print(
                        f"Found tool text representation: '{content[:200]}...'"
                        )
                    break
    ...
    def test_compaction_preserves_tool_result_content_as_text(self):
        ...
        messages = [UnifiedMessage(role='user', content='', tool_results=[{
            'type': 'tool_result', 'tool_use_id': 'call_123', 'content':
            'IMPORTANT_DATA_12345'}]), UnifiedMessage(role='user', content=
            'What was in that result?')]
        print('Action: Building OpenCode payload without tools...')
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4', tools=None, conversation_id=
            'test-conv', profile_arn='arn:test', thinking_config=
        ...
        for msg in history:
            if 'userInputMessage' in msg:
                ...
                if 'IMPORTANT_DATA_12345' in content:
                    found_data = True
                    print(
                        f"Found preserved data in history: '{content[:100]}...'"
                        )
                    break
        if not found_data:
            current_content = payload['conversationState']['currentMessage'][
                'userInputMessage'].get('content', '')
    ...
    def test_with_tools_defined_keeps_tool_structure(self):
        ...
        messages = [UnifiedMessage(role='user', content='Call a tool'),
            ...
            'test_tool', 'arguments': '{}'}}]), UnifiedMessage(role='user',
            content='', tool_results=[{'type': 'tool_result', 'tool_use_id':
            'call_123', 'content': 'Tool executed'}]), UnifiedMessage(role=
            'user', content='(empty placeholder)')]
        tools = [UnifiedTool(name='test_tool', description='A test tool',
            input_schema={'type': 'object', 'properties': {}})]
        print('Action: Building OpenCode payload WITH tools...')
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4', tools=tools, conversation_id=
            'test-conv', profile_arn='arn:test', thinking_config=
            ThinkingConfig(enabled=False))
    ...
    def test_empty_tools_list_triggers_stripping(self):
        ...
        messages = [UnifiedMessage(role='assistant', content='', tool_calls
            =[{'id': 'call_123', 'type': 'function', 'function': {'name':
            'some_tool', 'arguments': '{}'}}]), UnifiedMessage(role='user',
            content='(empty placeholder)')]
        print('Action: Building OpenCode payload with empty tools list...')
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4', tools=[], conversation_id=
            'test-conv', profile_arn='arn:test', thinking_config=
            ThinkingConfig(enabled=False))
...
class TestBuildOpenCodePayloadImages:
    ...
    def test_includes_multiple_images_in_current_message(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Compare these images', images=[{'media_type': 'image/jpeg',
            'data': 'image1_data'}, {'media_type': 'image/png', 'data':
            'image2_data'}, {'media_type': 'image/gif', 'data':
            'image3_data'}])]
        print('Action: Building OpenCode payload...')
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4', tools=None, conversation_id=
    ...
    def test_includes_images_in_history(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            "What's in this image?", images=[{'media_type': 'image/jpeg',
            'data': 'history_image_data'}]), UnifiedMessage(role=
            'assistant', content='I see a cat in the image.'),
    ...
    def test_images_with_tools(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            'Analyze this image and use tools if needed', images=[{
            'media_type': 'image/png', 'data': 'image_with_tools_data'}])]
        tools = [UnifiedTool(name='analyze_image', description=
            'Analyze an image', input_schema={'type': 'object',
            'properties': {}})]
        print('Action: Building OpenCode payload with tools...')
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4', tools=tools, conversation_id=
    ...
    def test_images_with_tool_results(self):
        ...
        messages = [UnifiedMessage(role='user', content='Call a tool'),
            UnifiedMessage(role='assistant', content='', tool_calls=[{'id':
            'call_123', 'type': 'function', 'function': {'name': 'get_data',
            'arguments': '{}'}}]), UnifiedMessage(role='user', content=
            "Here's the result and an image", images=[{'media_type':
            'image/jpeg', 'data': 'image_with_result_data'}], tool_results=
            [{'type': 'tool_result', 'tool_use_id': 'call_123', 'content':
            'Tool output'}])]
        tools = [UnifiedTool(name='get_data', description='Get data',
            input_schema={'type': 'object', 'properties': {}})]
        print('Action: Building OpenCode payload...')
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4', tools=tools, conversation_id=
    ...
    def test_no_images_when_none_provided(self):
        ...
        if 'images' in context:
            assert context['images'] == [
                ], 'Images should be empty when none provided'
        else:
    ...
    def test_large_image_data_preserved(self):
        ...
        large_image_data = 'A' * 500000
        messages = [UnifiedMessage(role='user', content=
            'Analyze this large image', images=[{'media_type': 'image/png',
    ...
    def test_images_with_thinking_injection(self):
        ...
        messages = [UnifiedMessage(role='user', content=
            "What's in this image?", images=[{'media_type': 'image/jpeg',
            'data': 'thinking_test_image'}])]
        print('Action: Building OpenCode payload with thinking injection...')
        with patch('opencode_zen.converters_core.FAKE_REASONING_ENABLED', True
            ):
            with patch('opencode_zen.converters_core.FAKE_REASONING_MAX_TOKENS'
                , 4000):
                result = build_opencode_payload(messages=messages,
                    system_prompt='', model_id='claude-sonnet-4', tools=
                    None, conversation_id='test-conv', profile_arn=
                    'arn:test', thinking_config=ThinkingConfig(enabled=True))
        current_msg = result.payload['conversationState']['currentMessage'][
            'userInputMessage']
        print(
            'Checking that images are directly in userInputMessage (Issue #32 fix)...'
        ...
        assert 'images' in current_msg
        assert len(current_msg['images']) == 1
        assert current_msg['images'][0]['source']['bytes'
            ] == 'thinking_test_image'
...
class TestValidateToolNames:
    ...
    def test_accepts_short_tool_names(self):
        ...
        try:
            from opencode_zen.converters_core import validate_tool_names
    ...
    def test_rejects_multiple_long_names(self):
        ...
        tools = [UnifiedTool(name='a' * 65, description='Test 1'),
            UnifiedTool(name='short', description='Test 2'), UnifiedTool(
            name='b' * 70, description='Test 3')]
        print('Action: Validating tool names (should raise ValueError)...')
        try:
            from opencode_zen.converters_core import validate_tool_names
            validate_tool_names(tools)
    ...
    def test_error_message_includes_solution(self):
        ...
        tools = [UnifiedTool(name='mcp__GitHub__' + 'a' * 60, description=
            'Test')]
        print('Action: Validating tool names (should raise ValueError)...')
        try:
            from opencode_zen.converters_core import validate_tool_names
    ...
    def test_real_world_mcp_tool_names(self):
        ...
        tools = [UnifiedTool(name=name, description='Test') for name in
            problematic_names]
        print(
            'Action: Validating real MCP tool names (should raise ValueError)...'
            )
        try:
            from opencode_zen.converters_core import validate_tool_names
            validate_tool_names(tools)
...
class TestGetTruncationRecoverySystemAddition:
    ...
    def test_returns_text_when_enabled(self):
        ...
        with patch.dict(os.environ, {'TRUNCATION_RECOVERY': 'true'}):
            ...
            from opencode_zen import config
            reload(config)
            from opencode_zen.converters_core import get_truncation_recovery_system_addition
...
class TestInjectThinkingTagsWithConfig:
    ...
    def test_disabled_by_global_flag(self, monkeypatch):
        ...
        monkeypatch.setattr(
            'opencode_zen.converters_core.FAKE_REASONING_ENABLED', False)
    ...
    def test_uses_default_budget(self, monkeypatch):
        ...
        """
        print(
            'Setting FAKE_REASONING_ENABLED=True, FAKE_REASONING_MAX_TOKENS=4000...'
            )
        monkeypatch.setattr(
            'opencode_zen.converters_core.FAKE_REASONING_ENABLED', True)
        monkeypatch.setattr(
            'opencode_zen.converters_core.FAKE_REASONING_MAX_TOKENS', 4000)
        monkeypatch.setattr(
        ...
        result = inject_thinking_tags(content, config)
        print(
            f'Checking for <max_thinking_length>4000</max_thinking_length>...')
        assert '<max_thinking_length>4000</max_thinking_length>' in result
        assert '<thinking_mode>enabled</thinking_mode>' in result
    ...
    def test_uses_custom_budget(self, monkeypatch):
        ...
        result = inject_thinking_tags(content, config)
        print(
            f'Checking for <max_thinking_length>8000</max_thinking_length>...')
    ...
    def test_applies_cap_with_warning(self, monkeypatch):
        ...
        content = 'Test content'
        print(
            f'Calling inject_thinking_tags with budget=50000 (exceeds cap)...')
        with patch('opencode_zen.converters_core.logger.warning'
            ) as mock_warning:
            result = inject_thinking_tags(content, config)
            print(
    ...
    def test_uses_budget_when_below_cap(self, monkeypatch):
        ...
        result = inject_thinking_tags(content, config)
        print(
            f'Checking for <max_thinking_length>5000</max_thinking_length>...')
    ...
    def test_cap_disabled_when_zero(self, monkeypatch):
        ...
        content = 'Test content'
        print(
            f'Calling inject_thinking_tags with budget=50000 (cap disabled)...'
            )
        result = inject_thinking_tags(content, config)
        print(
            f'Checking for <max_thinking_length>50000</max_thinking_length>...'
            )
...
class TestBuildOpenCodePayloadWithThinkingConfig:
    ...
    def test_passes_thinking_config_to_inject(self, monkeypatch):
        """
        What it does: Verifies that build_opencode_payload passes thinking_config to inject_thinking_tags
        ...
        print(
            f'Calling build_opencode_payload with thinking_config={thinking_config}...'
            )
        result = build_opencode_payload(messages=messages, system_prompt='',
            model_id='claude-sonnet-4.5', tools=None, conversation_id=
            'test-conv-123', profile_arn='arn:aws:test', thinking_config=
            thinking_config)
