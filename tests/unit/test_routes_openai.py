...
from opencode_zen.routes_openai import verify_api_key, router
from opencode_zen.config import PROXY_API_KEY, APP_VERSION
...
class TestVerifyApiKey:
    ...
    async def test_bearer_with_extra_spaces_raises_401(self):
        ...
        malformed = f'Bearer  {PROXY_API_KEY}'
        print('Action: Calling verify_api_key...')
        with pytest.raises(HTTPException) as exc_info:
...
class TestRootEndpoint:
    ...
    def test_root_returns_gateway_message(self, test_client):
        ...
        print(f'Result: {response.json()}')
        assert response.status_code == 200
        assert 'OpenCode Zen Gateway' in response.json()['message']
...
class TestModelsEndpoint:
    ...
    def test_models_rejects_invalid_key(self, test_client,
        ...
        response = test_client.get('/v1/models', headers={'Authorization':
            f'Bearer {invalid_proxy_api_key}'})
        print(f'Status: {response.status_code}')
        assert response.status_code == 401
    ...
    def test_models_contains_available_models(self, test_client,
        valid_proxy_api_key):
        """
        ...
        assert len(model_ids
            ) >= 1, 'Expected at least one model (hidden models)'

    def test_models_format_is_openai_compatible(self, test_client,
        valid_proxy_api_key):
        """
...
class TestChatCompletionsAuthentication:
    ...
    def test_chat_completions_requires_authentication(self, test_client):
        ...
        response = test_client.post('/v1/chat/completions', json={'model':
            'claude-sonnet-4-5', 'messages': [{'role': 'user', 'content':
            'Hello'}]})
        print(f'Status: {response.status_code}')
        assert response.status_code == 401

    def test_chat_completions_rejects_invalid_key(self, test_client,
        invalid_proxy_api_key):
        """
        ...
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {invalid_proxy_api_key}'}, json={
...
class TestChatCompletionsValidation:
    ...
    def test_validates_invalid_json(self, test_client, valid_proxy_api_key):
        ...
        response = test_client.post('/v1/chat/completions', headers={
            ...
            'Content-Type': 'application/json'}, content=b'not valid json {{{}'
            )
        print(f'Status: {response.status_code}')
        assert response.status_code == 422
    ...
    def test_validates_invalid_role(self, test_client, valid_proxy_api_key):
        ...
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role':
            'invalid_role', 'content': 'Hello'}]})
        print(f'Status: {response.status_code}')
        assert response.status_code != 422
    ...
    @pytest.mark.skip(reason=
        'Obsolete after OpenAI payload and Account System migration')
    def test_accepts_message_without_content(self, test_client,
        valid_proxy_api_key):
        """
        ...
        """
        print(
            'Action: POST /v1/chat/completions with message without content...'
            )
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user'}]})
        print(f'Status: {response.status_code}')
        assert response.status_code != 422 or 'content' not in str(response
            .json())
...
class TestChatCompletionsWithTools:
    ...
    @pytest.mark.skip(reason=
        'Obsolete after OpenAI payload and Account System migration')
    def test_accepts_valid_tool_definition(self, test_client,
        valid_proxy_api_key, sample_tool_definition):
        """
        ...
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user',
            'content': "What's the weather?"}], 'tools': [
    ...
    def test_accepts_multiple_tools(self, test_client, valid_proxy_api_key):
        ...
        tools = [{'type': 'function', 'function': {'name': 'get_weather',
            ...
            'properties': {}}}}, {'type': 'function', 'function': {'name':
            'get_time', 'description': 'Get time', 'parameters': {'type':
            'object', 'properties': {}}}}]
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user',
            'content': 'Hello'}], 'tools': tools})
        print(f'Status: {response.status_code}')
        assert response.status_code != 422
...
class TestChatCompletionsOptionalParams:
    ...
    @pytest.mark.skip(reason=
        'Obsolete after OpenAI payload and Account System migration')
    def test_accepts_max_tokens_parameter(self, test_client,
        valid_proxy_api_key):
        """
...
class TestChatCompletionsMessageTypes:
    ...
    def test_accepts_system_message(self, test_client, valid_proxy_api_key):
        ...
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'system',
            'content': 'You are helpful.'}, {'role': 'user', 'content':
            'Hello'}]})
        print(f'Status: {response.status_code}')
        assert response.status_code != 422
    ...
    def test_accepts_assistant_message(self, test_client, valid_proxy_api_key):
        ...
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user',
            'content': 'Hello'}, {'role': 'assistant', 'content':
...
class TestHTTPClientSelection:
    ...
    def test_streaming_uses_per_request_client(self,
        ...
        try:
            test_client.post('/v1/chat/completions', headers={
                'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
                'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user',
                'content': 'Hello'}], 'stream': True})
        except Exception:
...
class TestTruncationRecoveryMessageModification:
    ...
    def test_modifies_tool_result_with_truncation_notice(self):
        ...
        from opencode_zen.models_openai import ChatMessage
        tool_call_id = 'tooluse_test123'
        save_tool_truncation(tool_call_id, 'write_to_file', {'size_bytes': 
            5000, 'reason': 'test'})
        print('Setup: Creating request with tool_result...')
        messages = [ChatMessage(role='tool', tool_call_id=tool_call_id,
        ...
        modified_messages = []
        for msg in messages:
            if (msg.role == 'tool' and msg.tool_call_id and
                ...
                if truncation_info:
                    ...
                    synthetic = generate_truncation_tool_result(truncation_info
                        .tool_name, truncation_info.tool_call_id,
...
{msg.content}"""
    ...
    def test_no_modification_when_no_truncation(self):
        ...
        from opencode_zen.models_openai import ChatMessage
        messages = [ChatMessage(role='tool', tool_call_id=
            'tooluse_nonexistent', content='Success')]
    ...
    def test_pydantic_immutability_new_object_created(self):
        ...
        save_tool_truncation(tool_call_id, 'tool', {'size_bytes': 1000,
            'reason': 'test truncation'})
        original_msg = ChatMessage(role='tool', tool_call_id=tool_call_id,
            content='original')
        original_content = original_msg.content
        print('Action: Processing message...')
        from opencode_zen.truncation_recovery import should_inject_recovery, generate_truncation_tool_result
        from opencode_zen.truncation_state import get_tool_truncation
        if (original_msg.role == 'tool' and original_msg.tool_call_id and
            should_inject_recovery()):
...
class TestTruncationRecoveryEdgeCases:
    ...
    def test_orphaned_tool_result_no_crash(self):
        ...
        from opencode_zen.models_openai import ChatMessage
        messages = [ChatMessage(role='tool', tool_call_id=
            'tooluse_nonexistent_orphan', content='Result')]
...
{msg.content}"""
    ...
    def test_very_long_content_hash_uses_first_500_chars(self):
        ...
        from opencode_zen.truncation_state import save_content_truncation, get_content_truncation
    ...
    def test_recovery_disabled_cache_entry_remains(self):
        ...
        from opencode_zen.truncation_state import save_tool_truncation, get_cache_stats
        from opencode_zen.models_openai import ChatMessage
        import os
...
class TestContentTruncationRecovery:
    ...
    def test_adds_synthetic_user_message_after_truncated_assistant(self):
        ...
        modified_messages = []
        for msg in messages:
            if msg.role == 'assistant' and msg.content and isinstance(msg.
                content, str):
                ...
                if truncation_info:
                    ...
                    modified_messages.append(msg)
                    synthetic_user_msg = ChatMessage(role='user', content=
    ...
    def test_content_hash_matches_first_500_chars(self):
        ...
        print(
            'Action: Retrieving with request content (same first 500 chars)...'
            )
...
class TestWebSearchAutoInjectionOpenAI:
    ...
    def test_auto_injection_logic_openai(self):
        ...
        if WEB_SEARCH_ENABLED:
            has_ws = any(getattr(tool, 'type', None) == 'function' and 
                getattr(getattr(tool, 'function', None), 'name', None) ==
                'web_search' for tool in tools)
            if not has_ws:
                web_search_tool = Tool(type='function', function=
                    ToolFunction(name='web_search', description=
    ...
    def test_no_duplicate_injection_logic_openai(self):
        ...
        from opencode_zen.models_openai import Tool, ToolFunction
        existing_tools = [Tool(type='function', function=ToolFunction(name=
            'web_search', description='Existing web search', parameters={
            'type': 'object', 'properties': {}}))]
        WEB_SEARCH_ENABLED = True
        if WEB_SEARCH_ENABLED:
            has_ws = any(getattr(tool, 'type', None) == 'function' and 
                getattr(getattr(tool, 'function', None), 'name', None) ==
                'web_search' for tool in existing_tools)
            if not has_ws:
                existing_tools.append(Tool(type='function', function=
                    ToolFunction(name='web_search', description=
                    'Auto-injected', parameters={'type': 'object',
                    'properties': {}})))
        print(f'Checking: Only one web_search tool...')
        web_search_count = sum(1 for t in existing_tools if t.type ==
            'function' and t.function.name == 'web_search')
...
class TestModelsEndpointAccountSystem:
    ...
    def test_get_models_account_system_logic(self):
        ...
        if account_system:
            available_model_ids = (mock_account_manager.
        ...
        print(
            '✅ Account system mode correctly collects models from all accounts'
            )
...
class TestChatCompletionsFailoverLoop:
    ...
    async def test_chat_completions_failover_get_next_account(self):
        ...
        print(
            'Checking: get_next_account() called with exclude_accounts parameter...'
            )
        await mock_manager.get_next_account('claude-opus-4.5',
    ...
    async def test_chat_completions_failover_success_first_account(self):
        ...
        mock_account = Account(id='/home/user/account1.json', failures=0,
            last_failure_time=0.0, models_cached_at=time.time(), stats=
            AccountStats())
        mock_manager = Mock()
        mock_manager.get_next_account = AsyncMock(return_value=mock_account)
    ...
    async def test_chat_completions_failover_recoverable_try_next(self):
        ...
        await mock_manager.report_failure('/home/user/account1.json',
            'claude-opus-4.5', ErrorType.RECOVERABLE, 429, None)
    ...
    async def test_chat_completions_failover_fatal_immediate_return(self):
        ...
        mock_manager.report_failure = AsyncMock()
        await mock_manager.report_failure('/home/user/account1.json',
            'claude-opus-4.5', ErrorType.FATAL, 400,
    ...
    def test_chat_completions_failover_single_account_original_error(self):
        ...
        if len(all_accounts) == 1:
            error_response = {'status_code': last_error_status, 'detail':
                last_error_message}
        else:
            error_response = {'status_code': 503, 'detail':
    ...
    def test_chat_completions_failover_multi_account_generic_error(self):
        ...
        assert 'Error from last account: Token expired' in error_response[
            'detail']
    ...
    async def test_chat_completions_failover_all_unavailable(self):
        ...
        mock_manager._accounts = {'/home/user/account1.json': Mock(),
