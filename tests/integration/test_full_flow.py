...
class TestFullChatCompletionFlow:
    ...
    def test_full_flow_health_to_models_to_chat(self, test_client,
        valid_proxy_api_key):
        """
        ...
        models_response = test_client.get('/v1/models', headers={
        ...
        chat_response = test_client.post('/v1/chat/completions', headers={
    ...
    def test_authentication_flow(self, test_client, valid_proxy_api_key,
        ...
        wrong_auth_response = test_client.get('/v1/models', headers={
            'Authorization': f'Bearer {invalid_proxy_api_key}'})
        ...
        valid_auth_response = test_client.get('/v1/models', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'})
...
class TestRequestValidationFlow:
    ...
    def test_complex_message_formats(self, test_client, valid_proxy_api_key):
        ...
        system_user = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
        ...
        multi_turn = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user',
        ...
        with_tools = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}'}, json={
            'model': 'claude-sonnet-4-5', 'messages': [{'role': 'user',
            'content': "What's the weather?"}], 'tools': [{'type':
...
class TestErrorHandlingFlow:
    ...
    def test_wrong_content_type_handling(self, test_client, valid_proxy_api_key
        ...
        response = test_client.post('/v1/chat/completions', headers={
            'Authorization': f'Bearer {valid_proxy_api_key}',
            'Content-Type': 'text/plain'}, content=b'Hello')
        assert response.status_code == 422
...
class TestModelsEndpointIntegration:
    ...
    def test_models_returns_all_available_models(self, test_client,
        valid_proxy_api_key):
        """
    ...
    def test_models_caching_behavior(self, test_client, valid_proxy_api_key):
        ...
        response1 = test_client.get('/v1/models', headers={'Authorization':
            f'Bearer {valid_proxy_api_key}'})
        assert response1.status_code == 200
        print('Second models request...')
        response2 = test_client.get('/v1/models', headers={'Authorization':
            f'Bearer {valid_proxy_api_key}'})
...
class TestStreamingFlagHandling:
    ...
    def test_stream_true_accepted(self, test_client, valid_proxy_api_key):
        ...
        with patch('opencode_zen.routes_openai.OpenCodeHttpClient'
            ) as MockHttpClient:
            mock_client_instance = AsyncMock()
            mock_client_instance.request_with_retry = AsyncMock(return_value
                =mock_response)
...
class TestHealthEndpointIntegration:
    ...
    def test_root_and_health_consistency(self, test_client):
        ...
        assert health_response.json()['status'] == 'healthy'
        assert root_response.json()['version'] == health_response.json()[
            'version']
