...
from opencode_zen.account_manager import AccountManager, Account, AccountStats
...
class TestAccountSystemFullFlow:
            ...
            async def mock_initialize(account_id):
                ...
                for model in model_resolver.get_available_models():
                    if model not in manager._model_to_accounts:
                        from opencode_zen.account_manager import ModelAccountList
    ...
    async def test_half_open_recovery_after_timeout(
        ...
        from opencode_zen.config import ACCOUNT_RECOVERY_TIMEOUT, ACCOUNT_MAX_BACKOFF_MULTIPLIER
    ...
    async def test_ttl_refresh_on_usage(
        ...
        from opencode_zen.config import ACCOUNT_CACHE_TTL
