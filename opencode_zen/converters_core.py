...
def build_opencode_payload(
    ...
    for msg in messages:
        ...
        if msg.images and msg.role == "user":
            content_list = []
            if msg.content:
                content_list.append({"type": "text", "text": str(msg.content)})
            ...
            msg_dict["content"] = str(extract_text_content(msg.content)) if msg.content else ""
    ...
    payload = {
        ...
        "stream": True # Force streaming for now, gateway will handle it
    }
...
def build_openai_payload(
    ...
    """Builds OpenAI API payload from unified format."""
    openai_messages = []
